...
def project_points_to_pixels(xyzrgb, K, RT, H, W, spread_radius=2):
    ...
    xyz_camera = world_to_camera(xyz_world, RT) # emjay: here, RT is cam2world.
    del xyz_world
    ...
    # emjay added ------------------
    depth_flat = torch.zeros(H * W, 1, dtype=torch.float64, device=xyzrgb.device)
    depth_flat = depth_flat.scatter_reduce(0, inds_flat.long()[:, None], depth_filter, reduce="amin", include_self=False)
    # -----------------------------
    ...
    # emjay modified --------------
    return (img_norm, pixel_weights, uv, depth_flat)
    # original ---------------------
