...
def get_args():
    ...
    args.output_dir = os.path.join("logs", f"{now}_{args.save_name}")
    return args
...
def log_validation(
    ...
    for i, video in enumerate(videos):
        ...
        if step is not None:
            save_dir = os.path.join(args.output_dir, f"sampled_images/{step:06}")
            os.makedirs(save_dir, exist_ok=True)   
            filename = os.path.join(save_dir, f"video_{prompt}.mp4")
        else:
            filename = os.path.join(args.output_dir, f"video_{prompt}.mp4")
...
def main(args):
    ...
    data_config = OmegaConf.load(args.data_cfg_path).data
    data_config.params.batch_size=args.train_batch_size
    data_config.params.data_gpu = accelerator.num_processes
    data_config.params.frame_height = args.height
    data_config.params.frame_width = args.width
    data_config.params.model_frames = args.max_num_frames
    data_config.params.input_frames = args.max_num_frames
    data_config.params.output_frames = args.max_num_frames
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            data_time = time.time() - _start_data_time
            models_to_accumulate = [controlnet]

            with accelerator.accumulate(models_to_accumulate):
                # jho add >>>>
                model_input = batch["jpg"]
                BF,C,H,W = model_input.shape
                B = args.train_batch_size
                F=BF//B
            
                model_input = encode_video(rearrange(model_input, "(B F) C H W -> B F C H W", B=B, F=F, C=C, H=H, W=W)).to(dtype=weight_dtype)
                controlnet_encoded_frames = rearrange(batch["cond_frames_without_noise"], "(B F) C H W -> B F C H W", B=B, F=F, C=C, H=H, W=W).to(dtype=weight_dtype)
                prompts = [""] * args.train_batch_size
                # jho add <<<<
            ...
            if accelerator.is_main_process:
                if args.validation_prompt is not None and (step + 1) % args.validation_steps == 0:
                    ...
                    for validation_prompt, validation_video in zip(validation_prompts, validation_videos):
                        ...
                        # jho added >>>>
                        controlnet_frames = controlnet_encoded_frames[0].cpu().detach()
                        controlnet_frames = np.uint8((controlnet_frames+1)/2 * 255).transpose(0,2,3,1)
                        # jho added <<<<
